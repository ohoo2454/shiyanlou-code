        ...
        if (maxwidth):
            ...
            if (maxheight):
                if (self.height > maxheight):
                    ...
                    fname = self.image.filename
                    self.image = self.resize((wsize, maxheight), Image.LANCZOS)
                    self.image.filename = fname
                    self.width, self.height = self.image.size
                    self.total_pixels = self.width * self.height
                    ret += 2
    ...
    def showSkinRegions(self):
        ...
        for pixel in self.skin_map:
            if pixel.id not in skinIdSet:
                simageDate[pixel.x, pixel.y] = 0, 0, 0
            else:
                simageDate[pixel.x, pixel.y] = 255, 255, 255
        ...
        simage.save('{}{}_{}{}'.format(fileDirectory, fileName, "Node" if self.result else "Normal", fileExtName))
