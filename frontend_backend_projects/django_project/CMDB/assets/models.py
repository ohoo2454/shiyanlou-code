...
class Asset(models.Model):
    ...
    status = models.SmallIntegerField(choices=asset_status_choices, default=0,
                                      verbose_name='设备状态')
    ...
    admin = models.ForeignKey(User, null=True, blank=True, 
                              verbose_name='资产管理员', related_name='admin',
                              on_delete=models.SET_NULL)
...
class CPU(models.Model):
    ...
    CPU组件
    """
    # 设备上的cpu肯定都是一样的，所以不需要建立多个cpu数据，一条就可以，
    # 因此使用一对一。
    asset = models.OneToOneField('Asset', on_delete=models.CASCADE)  
...
class RAM(models.Model):
    ...
    model = models.CharField('内存型号', max_length=128, blank=True, 
                             null=True)
...
class Disk(models.Model):
    ...
    slot = models.CharField('所在插槽位', max_length=64, blank=True, 
                            null=True)
    model = models.CharField('磁盘型号', max_length=128, blank=True, 
                             null=True)
    manufacturer = models.CharField('磁盘制造商', max_length=128, blank=True, 
                                    null=True)
    capacity = models.FloatField('磁盘容量(GB)', blank=True, null=True)
    interface_type = models.CharField('接口类型', max_length=16, 
                                      choices=disk_interface_type_choices, 
                                      default='unknown')
...
class NIC(models.Model):
    ...
    bonding = models.CharField('绑定地址', max_length=64, blank=True, 
                               null=True)
...
class EventLog(models.Model):
    ...
    asset = models.ForeignKey('Asset', blank=True, null=True, 
                              on_delete=models.SET_NULL)
    # 当资产审批失败时有这项数据
    new_asset = models.ForeignKey('NewAssetApprovalZone', blank=True, 
...
class NewAssetApprovalZone(models.Model):
    ...
    asset_type = models.CharField(choices=asset_type_choices, 
                                  default='server', max_length=64, blank=True,
                                  null=True, verbose_name='资产类型')
    ...
    os_type = models.CharField('系统类型', max_length=64, blank=True, 
                               null=True)
    os_release = models.CharField('操作系统版本号', max_length=64, blank=True,
                                  null=True)
