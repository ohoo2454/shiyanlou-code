...
class StarkHandler(object):
    ...
    def display_edit(self, obj=None, is_header=None):
        ...
        return mark_safe('<a href="%s">编辑</a>' % reverse(name, 
                args=(obj.pk,)))
    ...
    def display_del(self, obj=None, is_header=None):
        ...
        return mark_safe('<a href="%s">删除</a>' % reverse(name, 
                args=(obj.pk,)))
    ...
    def list_view(self, request, *args, **kwargs):
        ...
        for row in data_list:
            ...
            if display_list:
                for key_or_func in display_list:
                    if isinstance(key_or_func, FunctionType):
                        tr_list.append(key_or_func(self, obj=row, 
                                is_header=False))
                    else:
    ...
    def edit_view(self, request, pk):
        ...
        :return:
        """
        current_edit_obj = self.model_class.objects.filter(id=pk).first()
    ...
    def reverse_add_url(self):
        ...
        if not self.request.GET:
            ...
            add_url = '%s?%s' % (base_url, new_querydict.urlencode())
        return add_url
    ...
    def reverse_edit_url(self, *args, **kwargs):
        ...
        if not self.request.GET:
            ...
            new_querydict = QueryDict(mutable=True)
            new_querydict['_filter'] = param
            edit_url = '%s?%s' % (base_url, new_querydict.urlencode())
        return edit_url
    ...
    def reverse_del_url(self, *args, **kwargs):
        ...
        if not self.request.GET:
            ...
            new_querydict = QueryDict(mutable=True)
            new_querydict['_filter'] = param
            del_url = '%s?%s' % (base_url, new_querydict.urlencode())
        return del_url
    ...
    def get_urls(self):
        ...
        patterns = [
            ...
            url(r'^edit/(\d+)/$', self.wrapper(self.edit_view), 
                    name=self.get_edit_url_name),
            url(r'^del/(\d+)/$', self.wrapper(self.del_view), 
